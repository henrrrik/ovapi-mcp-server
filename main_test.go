...
// startTestServerWithLog is startTestServer with the access log captured.
...
// decodeServerName reads an initialize response and returns serverInfo.name.
func decodeServerName(t *testing.T, resp *http.Response) string {
	t.Helper()
	...
	return out.Result.ServerInfo.Name
}
...
// Streamable HTTP is served at /mcp so clients that don't need a long-lived
...
func TestStreamableHTTP_InitializeAtMCPEndpoint(t *testing.T) {
	...
	if name := decodeServerName(t, resp); name != "ovapi-mcp-server" {
		t.Errorf("serverInfo.name = %q, want ovapi-mcp-server", name)
	}
}
...
func TestStreamableHTTP_PostToSSEPathIsServed(t *testing.T) {
	...
	for _, path := range []string{"/sse", "/sse/"} {
		...
		if resp.StatusCode != http.StatusOK {
			resp.Body.Close()
			t.Errorf("POST %s initialize: status %d", path, resp.StatusCode)
			continue
		}
		if name := decodeServerName(t, resp); name != "ovapi-mcp-server" {
			t.Errorf("POST %s: serverInfo.name = %q", path, name)
		}
	}
}
